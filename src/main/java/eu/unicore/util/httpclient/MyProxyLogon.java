 ...
 * a MyProxy server. Authentication to MyProxy is done via username/password.
 * <p/>
 ...
 * <code>getPrivateKey</code> to access the retrieved credentials. 
 * <p/>
 * When you are done, call <code>diconnect()</code> to clean up resources and 
 * close the socket connection to myproxy.
 * <p/>
 * (this is based on the original MyProxyLogon class from java cog kit. It was 
 * modified for use with UNICORE by Bernd Schuller and Shahbaz Memon)
 *
...
public class MyProxyLogon {
	...
	/**
	 * Sets the key size. Defaults to 2048 bit.
	 *
	 * @param keySize
	 */
	...
	// for unit testing
	public static PrivateKey testingPrivateKey;
	 ...
	 * SSL authentication must be configured beforehand 
	 * via {@link #setValidator(X509CertChainValidator)}
	 */
	...
	public void logon() throws IOException, GeneralSecurityException {
		...
		while ((line = readLine(socketIn)) != null) {
			if (line.startsWith(TRUSTROOTS)) {
				// we don't support getting trust roots, but if the server 
				// sends them anyway, we should not fail
				String filenameList = line.substring(TRUSTROOTS.length());
				String[] trustrootFilenames = filenameList.split(",");
				String[] trustrootData = new String[trustrootFilenames.length];
	...
	public X509Certificate getCertificate() {
		return certificateChain != null ? certificateChain.iterator().next() : null;
	}
